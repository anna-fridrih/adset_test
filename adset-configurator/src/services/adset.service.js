...
class AdsetService {
    ...
    static async generateAdset(geo, device) {
        if (!this.tree)
            await this.initialize();
        const location = this.tree?.children?.find(n => n.conditions?.geo?.includes(geo));
        if (!location)
