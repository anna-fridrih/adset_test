import * as AdsetService from '../../src/services/adset.service'; // Импортируем весь объект с методами
import TreeNode from '../../src/types/tree';

// Мокаем конкретный метод через jest.spyOn
...
it('should generate adset correctly for geo=RU and device=mobile', async () => {
  const generateAdsetSpy = jest.spyOn(AdsetService.default, 'generateAdset')
    ...
    // expect(result.adset_id).toBe("12345");  // Добавлен тест для проверки adset_id
